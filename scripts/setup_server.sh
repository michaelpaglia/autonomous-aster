...
if [ "$setup_systemd" = "y" ]; then
    ...
    sed -i "s|User=your-username|User=$USER|g" deploy/cosmic-trader.service
    sed -i "s|WorkingDirectory=/path/to/aster-dex|WorkingDirectory=$CURRENT_DIR|g" deploy/cosmic-trader.service
    sed -i "s|/path/to/venv/bin/python|$VENV_PYTHON|g" deploy/cosmic-trader.service
    sed -i "s|/path/to/aster-dex|$CURRENT_DIR|g" deploy/cosmic-trader.service

    # Copy service file
    sudo cp deploy/cosmic-trader.service /etc/systemd/system/
